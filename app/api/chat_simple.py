...
async def send_chat_message(message_data: ChatMessage, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        chat_response = ChatResponse(
            ...
            voice_id=voice_id,
            voice_info={
                "name": voice_config["description"].split()[0],  # Extract first word as name
                "language": voice_config["language"],
                "gender": "female" if "female" in voice_config["description"].lower() else "male"
            },
...
async def get_available_agents():
    ...
    for agent_type, config in AGENT_CONFIGS.items():
        ...
        agents.append(AgentInfo(
            ...
            voice_id=voice_id,
            voice_name=voice_config["description"].split()[0],  # Extract first word as name
...
async def generate_voice_only(voice_request: VoiceRequest):
    ...
    try:
        ...
        audio_chunks = []
        async for chunk in murf_client.stream_text_to_speech(voice_request.text, voice_request.agent_type, voice_request.user_id):
            audio_chunks.append(chunk)
        
        if audio_chunks:
            audio_data = b''.join(audio_chunks)
            voice_config = get_agent_voice(voice_request.agent_type)
    ...
    try:
        ...
        if audio_chunks:
            ...
            return {
                ...
                "voice_id": voice_config["voice_id"],
                "message": f"Voice generated by {voice_config['description']}"
            }
        else:
            raise HTTPException(status_code=500, detail="No audio generated")
...
async def generate_voice_audio(text: str, agent_type: str, user_id: str):
    ...
    try:
        ...
        chunk_count = 0
        async for chunk in murf_client.stream_text_to_speech(text, agent_type, user_id):
            chunk_count += 1
